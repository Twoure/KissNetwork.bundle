...
# ------------------------------------------------------------
# Edited by Twoure for Plex Media Server

def aadecode(aastring):
    ...
    decodestring = Regex(r"(?si)\\\+([^(]+)").search(aastring).group(1)
...
def url_from_aastring(decodestring):
    import re
...
def decode(encoded):
    #for octc in (c for c in Regex(r'\\(\d{2,3})').findall(encoded)):
